...
def create_hdf5_dataset(args):
    ...
    index_dataset = np.ndarray(shape=(0), dtype=np.uint32)
    max_index_dataset = np.ndarray(shape=(0), dtype=np.uint32)
    subjects = []
    ...
    for idx, current_subject in enumerate(subject_dir):
        ...
        index_dataset = np.append(index_dataset, np.arange(CT_data.shape[2], dtype=np.uint32), axis=0)
        max_index_dataset = np.append(max_index_dataset,
                                      np.full(CT_data.shape[2], CT_data.shape[2] - 1, dtype=np.uint32), axis=0)
    ...
    index_dataset = np.transpose(np.vstack((index_dataset, max_index_dataset))).astype(np.uint32)
