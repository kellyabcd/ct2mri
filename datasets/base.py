...
class multi_ch_nifti_default_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.to_normal:
            image = (image - 0.5) * 2.
            image.clamp_(-1., 1.)
        # 在 base.py 的 __getitem__ 方法中
        # print(f"index: {index}, slice_number: {slice_number}, radius: {self.radius}")
        # print(f"images shape: {self.images.shape}")
        # print(f"slice range: {index - slice_number} to {index + self.radius + 1}")
