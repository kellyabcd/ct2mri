...
class hist_context_BraTS_Paired_Dataset(Dataset):
    def __init__(self, dataset_config, stage='train'):
        ...
        with h5py.File(hdf5_path, "r") as hf:
            ...
            index_dataset = np.array(hf.get('index_dataset')).astype(np.uint32)
...
class CT2MR_Paired_Dataset(Dataset):
    def __init__(self, dataset_config, stage='train'):
        ...
        with h5py.File(hdf5_path, "r") as hf:
            ...
            index_dataset = np.array(hf.get('index_dataset')).astype(np.uint32)
            subjects = np.array(hf.get("subject"))
            print(f"A_dataset shape: {A_dataset.shape}")  # 应不为空
            print(f"B_dataset shape: {B_dataset.shape}")
            print(f"index_dataset shape: {index_dataset.shape}")
...
print("custom.py 加载完成，已注册的数据集类型：", Registers.datasets.dict.keys())
