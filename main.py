import sys
import os
# 将项目根目录（CT2MRI-main）添加到 Python 路径
sys.path.append(os.path.dirname(os.path.abspath(__file__)))
print("当前添加的路径：", os.path.dirname(os.path.abspath(__file__)))
print("Python 路径列表：", sys.path)
import argparse
import os
import yaml
import copy
...
print("强制导入后已注册的数据集：", list(Registers.datasets.keys()))  # 应显示 ct2mr_aligned 等
...
def parse_args_and_config():
    ...
    parser.add_argument('--dataset_path', type=str, default=None,
                        help='Path to the dataset directory (override config)')
