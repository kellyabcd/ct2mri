...
class BBDMRunner(DiffusionBaseRunner):
    ...
    def sample(self, net, batch, sample_path, stage='train'):
        ...
        print(f"sample stats: max={sample.max()}, min={sample.min()}, has_nan={torch.isnan(sample).any()}")
    ...
    def sample_to_eval(self, net, test_dataset, sample_path):
        ...
        raw_data_dir = "/home/user/Brownian-Bridge-Diffusion-Model/Syna2023/3d"
        ...
        for idx in range(len(test_dataset)):
            ...
            # 获取原始pid路径（可能包含混合分隔符）
            pid_full_path = test_dataset[idx][0][1].decode('utf-8')

            # 关键：统一替换分隔符（将 \ 换成 /，避免转义问题）
            normalized_path = pid_full_path.replace('\\', '/')

            # 分割路径并过滤空字符串（处理可能的结尾斜杠）
            path_parts = [part for part in normalized_path.split('/') if part]

            # 取最后一个非空部分（即文件夹名）
            pid_last_folder = path_parts[-1] if path_parts else ''

            print("pid最后一个文件夹:", pid_last_folder)  # 应输出 "1BA022"
            batch_dict[pid_last_folder].append(test_dataset[idx])
        ...
        for pid in tqdm(batch_dict.keys()):
            ...
            if os.path.exists(out_path):
                ...
                # syn_img = np.squeeze(syn_img)
                raw_img = nib.load(os.path.join(raw_data_dir, pid, f"mri_preprocessed.nii")).get_fdata()
                raw_img = np.nan_to_num(raw_img)
                print("生成的shape：",syn_img.shape)
                print("原始的shape：",raw_img.shape)
            ...
            print(f"pid={pid}: x通道数={x.shape[1]}, x_cond通道数={x_cond.shape[1]}")
            context = context[0] if context else None
            ...
            if context is not None:
                ...
                print(f"pid={pid}: context通道数={context[0].shape[1]}")
            print(f"pid={pid}: 总通道数={x.shape[1] + x_cond.shape[1] + (context[0].shape[1] if context else 0)}")
            ...
            raw_image_path = os.path.join(raw_data_dir, pid, f"mr_preprocessed.nii")
            print("raw_MRimage_path:", raw_image_path)
            if 'BraTS' in sample_path:
            ...
            # syn_img = np.squeeze(syn_img)
            calcul_metrics(metrics_dict, pid, syn_img, raw_img)
        metrics_columns = ['PSNR', 'SSIM', 'MAE', 'MSE']
        df = pd.DataFrame.from_dict(metrics_dict, orient='index', columns=metrics_columns)
        means = df.mean()
        df.loc['mean'] = means
                
        df.to_csv(os.path.join(sample_path, 'results.csv'), index_label='pa_id')
        
        results_file = os.path.join('/home/user/Brownian-Bridge-Diffusion-Model/CT2MRI-main/results/test_results.csv')
